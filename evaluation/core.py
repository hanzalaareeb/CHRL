...
def evaluate_checkpoint(agent, actor_path, critic_path, seeds, config=Config, use_target=False):
    env = ContinuousHomeostaticEnv(config, resources=config.RESOURCES, regen_delay=config.REGEN_DELAY, survival_bonus=0.0)
...
def checkpoint_path_map(run_dir):
    ...
    return {
        "latest": (os.path.join(checkpoint_dir, "homeostatic_actor.pth"), os.path.join(checkpoint_dir, "homeostatic_critic.pth")),
        "best": (os.path.join(checkpoint_dir, "best_actor.pth"), os.path.join(checkpoint_dir, "best_critic.pth")),
        "stage1_best": (os.path.join(checkpoint_dir, "stage1_best_actor.pth"), os.path.join(checkpoint_dir, "stage1_best_critic.pth")),
        "stage2_best": (os.path.join(checkpoint_dir, "stage2_best_actor.pth"), os.path.join(checkpoint_dir, "stage2_best_critic.pth")),
        "stage3_best": (os.path.join(checkpoint_dir, "stage3_best_actor.pth"), os.path.join(checkpoint_dir, "stage3_best_critic.pth")),
        "stage4_best": (os.path.join(checkpoint_dir, "stage4_best_actor.pth"), os.path.join(checkpoint_dir, "stage4_best_critic.pth")),
    }
