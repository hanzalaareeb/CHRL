...
from evaluation.core import available_checkpoints, checkpoint_path_map, evaluate_checkpoint
...
def print_summary_table(results):
    ...
    for name, metrics in results:
        print(f"{name}\t{metrics['avg_survival']:.1f}\t{metrics['avg_consumption']:.1f}\t{metrics['avg_final_drive']:.2f}")


def print_diagnostic_block(name, metrics):
    print(
        f"\n[{name}] nearest={metrics['nearest_resource_distance']:.2f} | "
    ...
    print(
        f"[{name}] first_food={metrics['first_food_step']:.1f} | "
...
def main():
    ...
    for name in ["latest", "best", "stage1_best", "stage2_best", "stage3_best", "stage4_best"]:
        ...
        results.append((name, evaluate_checkpoint(agent, actor_path, critic_path, seeds, config=Config)))
