...
def collect(agent, env, n_episodes=10, gamma=Config.GAMMA, deterministic=True, use_target=False):
    ...
    for _ in range(n_episodes):
        ...
        for _ in range(env.max_steps):
            action = (env.action_space.sample() if agent is None else agent.select_action(state, add_noise=not deterministic, use_target=use_target))
    ...
    q_estimates = agent.evaluate_q(states, actions) if agent is not None else np.full(len(states), np.nan)
    ...
    return {
        ...
        "survival_bonus": env.survival_bonus,
    }
...
def analyze(data, config=Config, save_path=None):
    ...
    print(f"  per-step reward : mean={rewards.mean():.3f}  std={rewards.std():.3f}  min={rewards.min():.3f}  max={rewards.max():.3f}")
    ...
    telescoped = data["reward_scale"] * ((data["drive_initial"] - data["drive_final"]) - data["effort_sums"] + data["bonus_sums"])
    print(f"  telescoped return check scale*[(D_init-D_final) - effort + bonus_sum] : mean={np.nanmean(telescoped):.2f}  (matches episode return)")
    ...
    ax = axes[2]
    ax.bar(np.arange(len(ep_ret)), ep_ret, color=["tab:red" if r < 0 else "tab:green" for r in ep_ret])
    ...
    print("=============================================\n")
    return {"step_reward_mean": float(rewards.mean()), "frac_neg_episodes": frac_neg_eps, "critic_bias": bias, "critic_norm_bias": norm_bias}
...
def compare_actor_heads(agent, env, n_episodes=3, gamma=Config.GAMMA):
    ...
    print(f"[policy head check] actor mean return={actor_ret.mean():.2f}, survival={actor_survival:.1%} | actor_target mean return={target_ret.mean():.2f}, survival={target_survival:.1%}")
